...
export default function AdminLogin() {
    ...
    const [error, setError] = useState(() =>
        searchParams.get('expired') === '1' ? t('admin.login.sessionExpired') : ''
    );
        ...
        <div className="min-h-screen flex bg-slate-50 font-sans">
            <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-slate-900 via-black to-slate-900">
                ...
                <div className="relative z-10 flex flex-col justify-between p-14 text-white w-full">
                    ...
                    <p className="text-xs text-slate-400/70">© {new Date().getFullYear()} Waves. {t('admin.login.rights')}</p>
                </div>
            </div>
            ...
            <div className="flex-1 flex items-center justify-center p-6 sm:p-10">
                <div className="w-full max-w-sm">
                    ...
                    <div className="mb-8">
                        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{t('admin.login.welcomeBack')}</h1>
                        <p className="text-sm text-slate-500 mt-1.5">{t('admin.login.subtitle')}</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-xs font-semibold text-slate-600 mb-1.5">{t('admin.login.email')}</label>
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full rounded-xl border border-slate-200 pl-10 pr-3.5 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#81A6C6]/25 focus:border-[#81A6C6] transition-colors"
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-slate-600 mb-1.5">{t('admin.login.password')}</label>
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full rounded-xl border border-slate-200 pl-10 pr-10 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#81A6C6]/25 focus:border-[#81A6C6] transition-colors"
                                    placeholder="••••••••"
                                />
                                <button
                                    type="button"
