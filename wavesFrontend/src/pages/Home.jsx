...
const Home = () => {
    ...
    const displayCategories = featuredCategories.length > 0
        ? featuredCategories.slice(0, 6).map((cat, idx) => ({
            id: cat.id,
            name: cat.name || cat.name_en || cat.name_ar,
    ...
    const heroBadge = hero?.badge_text || t('home.heroBadge');
    const heroHeading = hero?.heading || t('home.heroHeading');
    const heroSubtext = hero?.subtext || t('home.heroSubtext');
    const heroButton1Text = hero?.button1_text || t('home.heroButton1');
    const heroButton1Link = hero?.button1_link || '/products';
    const heroButton2Text = hero?.button2_text || t('home.heroButton2');
                    ...
                    <div className="max-w-xl text-center lg:text-left">
                        ...
                        <div className="grid grid-cols-3 gap-4 pt-6 border-t border-black/10">
                            <div>
                                ...
                                <p className="text-[11px] font-semibold text-gray-600 uppercase tracking-wider">{t('home.statsBrands')}</p>
                            </div>
                            <div>
                                <h3 className="text-xl sm:text-2xl font-black text-black">{stats ? `${stats.products_count}+` : '—'}</h3>
                                <p className="text-[11px] font-semibold text-gray-600 uppercase tracking-wider">{t('home.statsProducts')}</p>
                            </div>
                            <div>
                                <h3 className="text-xl sm:text-2xl font-black text-black">{stats ? `${stats.customers_count}+` : '—'}</h3>
                                <p className="text-[11px] font-semibold text-gray-600 uppercase tracking-wider">{t('home.statsCustomers')}</p>
                            </div>
                        </div>
                    </div>
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <p className="text-xs text-gray-500 font-medium mt-1">{t('home.featuredBrandsSubtitle')}</p>
                    </div>
                    <Link
                        to="/brands"
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <p className="text-xs text-gray-500 font-medium mt-1">{t('home.trendingProductsSubtitle')}</p>
                    </div>
                    <Link
                        to="/products"
                        className="text-xs font-bold text-gray-900 hover:text-[#81A6C6] flex items-center gap-1 transition-colors"
                    ...
                    {galleryItems.length > 0 ? (
                        <div className="relative h-72 sm:h-96 w-full rounded-2xl overflow-hidden shadow-inner">
                            <img
                                ...
                                alt={t('home.galleryHighlightAlt')}
                            ...
                            <div className="absolute inset-x-0 bottom-0 bg-black/40 backdrop-blur-md p-6 border-t border-white/20 text-white">
                                <p className="text-xl sm:text-2xl font-black tracking-tight">
                                    {galleryItems[currentGalleryIndex]?.title || galleryItems[currentGalleryIndex]?.name || t('home.featuredHighlight')}
                                </p>
                            </div>
                        </div>
                    ) : (
                        <div className="h-48 flex items-center justify-center text-gray-400 font-medium">
                            {t('home.noGalleryItems')}
                        </div>
                    )}
                </div>
            </section>
