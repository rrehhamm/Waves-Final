...
const ProductCard = ({ product }) => {
    const {
        ...
        price = 145,
        oldPrice = null,
        // discount_percent/final_price come from the backend's ProductResource when an admin
        // has set a per-product discount (separate from the automatic first-order 20% discount)
        discount_percent = null,
        final_price = null,
        discount = discount_percent,
        // The backend's ProductResource returns the image as `main_image`, not `image` -
        // fall back through both so this card works whether it's fed raw API data or
        // an already-transformed object.
        main_image,
    ...
    return (
        <div className="group flex flex-col w-full bg-white rounded-2xl p-3 border border-gray-100 shadow-sm hover:shadow-xl hover:border-[#AACDDC] transition-all duration-300">
            ...
            <div className="mt-3 flex flex-col gap-1.5 px-1">
                ...
                {/* Price - shows the admin-set discounted final_price when one applies */}
                <div className="flex items-center gap-2 mt-1">
                    <span className="text-lg sm:text-xl font-black text-black tracking-tight">
                        JD{hasDiscount ? final_price : price}
                    </span>
